...
from source_fwd import (
    load_fits, Blurring, lens_to_params)
...
print('start rec')
data_space = R.target
N = ift.ScalingOperator(data_space, 1/noise_scale)
...
source_reconstruction = np.roll(
    np.roll(source_reconstruction[maxind[0]-maxi:maxind[0]+maxi,
            ...
            -shiftx, axis=0),
    -shifty, axis=1)
source_reconstruction = source_reconstruction[slicer, slicer]
...
ims[0, 0] = axes[0, 0].imshow(source['source'], origin='lower')
ims[0, 1] = axes[0, 1].imshow(source_reconstruction, origin='lower')
ims[0, 2] = axes[0, 2].imshow(
    source['source']-source_reconstruction, origin='lower', vmin=-0.1, vmax=0.4)# , cmap='RdBu_r')
